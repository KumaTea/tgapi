...
# GET
    ...
    class get:
        ...
        def group_admin(self, chat_id):
            ...
            get_admin = api_url + 'getChatAdministrators'
...
# POST
        ...
        def photo(self, photo, reply_to=False, upload=False):
            if upload:
                with open(photo, 'rb') as fl:
                    sending = {'photo': fl}
                    if reply_to:
                        msg_url = api_url + 'sendPhoto?chat_id=' + str(self.chat_id) + '&' + str(reply_to)
                    else:
                        msg_url = api_url + 'sendPhoto?chat_id=' + str(self.chat_id)
        ...
        def video(self, video, reply_to=False, upload=False):
            if upload:
                with open(video, 'rb') as fl:
                    sending = {'video': fl}
                    if reply_to:
                        msg_url = api_url + 'sendVideo?chat_id=' + str(self.chat_id) + '&' + str(reply_to)
                    else:
                        msg_url = api_url + 'sendVideo?chat_id=' + str(self.chat_id)
                    result = requests.post(msg_url, files=sending)
                return result.json()
            else:
                answer = {
        ...
        def gif(self, gif, reply_to=False, upload=False):
            if upload:
                with open(gif, 'rb') as fl:
                    ...
                    if reply_to:
                        msg_url = api_url + 'sendAnimation?chat_id=' + str(self.chat_id) + '&' + str(reply_to)
                    else:
                        msg_url = api_url + 'sendAnimation?chat_id=' + str(self.chat_id)
                    result = requests.post(msg_url, files=sending)
                return result.json()
            else:
                answer = {
        ...
        def file(self, file, reply_to=False, upload=False):
            if upload:
                with open(file, 'rb') as fl:
                    sending = {'document': fl}
                    if reply_to:
                        msg_url = api_url + 'sendDocument?chat_id=' + str(self.chat_id) + '&' + str(reply_to)
                    else:
                        msg_url = api_url + 'sendDocument?chat_id=' + str(self.chat_id)
                    result = requests.post(msg_url, files=sending)
                return result.json()
            else:
                answer = {
    ...
    class edit:
        ...
        def text(self, text):
            ...
            edit_text = api_url + 'editMessageText'
